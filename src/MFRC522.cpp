...
MFRC522::MFRC522(	volatile avr_gpio_registers_t * chipSelectPort,		///< Arduino pin connected to MFRC522's SPI slave select input (Pin 24, NSS, active low)
	...
	_chipSelectPad = chipSelectPad;
	_resetPowerDownPort = resetPowerDownPort;
	_resetPowerDownPad = resetPowerDownPad;
} // End constructor
...
void MFRC522::PCD_Init() {
	...
	palSetPadMode(_chipSelectPort, _chipSelectPad, PAL_MODE_OUTPUT_PUSHPULL);   //pinMode(_chipSelectPort, OUTPUT);
	palSetPad(_chipSelectPort, _chipSelectPad);   //digitalWrite(_chipSelectPort, HIGH);
	
	// If a valid pin number has been set, pull device out of power down / reset state.
	if (_resetPowerDownPort != 0) {
		// First set the resetPowerDownPin as digital input, to check the MFRC522 power down mode.
		palSetPadMode(_resetPowerDownPort, _resetPowerDownPad, PAL_MODE_INPUT);  //pinMode(_resetPowerDownPin, INPUT);
	
		if (palReadPad(_resetPowerDownPort, _resetPowerDownPad) == PAL_LOW){           //if (digitalRead(_resetPowerDownPin) == LOW) {	// The MFRC522 chip is in power down mode.
			palSetPadMode(_resetPowerDownPort, _resetPowerDownPad, PAL_MODE_OUTPUT_PUSHPULL);   //pinMode(_resetPowerDownPin, OUTPUT);		// Now set the resetPowerDownPin as digital output.
			palClearPad(_resetPowerDownPort, _resetPowerDownPad);    //digitalWrite(_resetPowerDownPin, LOW);		// Make shure we have a clean LOW state.
			chThdSleepMilliseconds(1);          //delayMicroseconds(2);	TODO			// 8.8.1 Reset timing requirements says about 100ns. Let us be generous: 2μsl
			palSetPad(_resetPowerDownPort, _resetPowerDownPad);   //digitalWrite(_resetPowerDownPin, HIGH);		// Exit power down mode. This triggers a hard reset.
			// Section 8.8.2 in the datasheet says the oscillator start-up time is the start up time of the crystal + 37,74μs. Let us be generous: 50ms.
			chThdSleepMilliseconds(50);   //delay(50);
			hardReset = true;
		}
	}
